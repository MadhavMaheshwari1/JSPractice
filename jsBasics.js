...
// TypeScript...
// Interfaces are open while type aliases are not...
// Meaning that Interfaces can be redeclared but not the same with type aliases...
// Declarations in Interfaces are merged...
// For eg: interface Point{x:number}
// interface Point{y:number}
// Merged Version: Point{x:number;y:number;}
// In typescript we can combine simple types to make complex ones...
// Two popular ways to do so: 1. Generics 2. Unions...
// Through Union you can have declare that a particular variable can have multiple types...
// Through generics we can declare different types of types...
// In TS if two objects have the same shape then they are considered to be of same type...
// The shape matching requires only a subset of object's fields to match...
// Compilation vs JIT: The machine code in JIT needs to be executed immediately whereas not the case in Compilation...
